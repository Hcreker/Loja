...
async function gerarPDF(users) {
    ...
    const docDefinition = {
        content: [
            { text: 'Relatório de Produto', style: 'header' },
            {
                table: {
                    headerRows: 1,
                    widths: ['auto', '*', '*',"*","*","*","*","*"],
                    body: [
                        ['ID', 'Nome', 'Descricao', 'Fornecedor', 'Marca', 'Compra', 'Venda', 'Estoque'],
                      ...users.map(user => [user.id, user.nome, user.descricao, user.fornecedor, user.marca, user.compra, user.venda, user.estoque]),
